...
function barMouseOver(d, i, x, y, bar, tooltip, user_id, time, metric, barWidth) {
	...
	if (time == "Months") {
	    tooltip.style("left", x(new Date(d.date)) + (xy.left - tooltip_width + tooltip_width/2 + barWidth + barWidth/2 + 15) + "px");     
	    tooltip.style("top", y(d.load_volume) - 70 - tooltip_height + xy.top + "px");
	} else {
		tooltip.style("left", x(new Date(d.date)) + (xy.left - tooltip_width + tooltip_width/2 + barWidth + barWidth/2) + "px");     
	    tooltip.style("top", y(d.load_volume) - 70 + xy.top + "px");
	}
